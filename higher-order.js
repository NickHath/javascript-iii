...
let postTaxPrices = prices.map((num) => 1.07 * num);
...
let totalPopulation  = populations.reduce((sum, currNum) => sum + currNum);
...
let ordersTotal = orders.map((currObj) => currObj["price"] * (1.0 + currObj["tax"]));
...
let bobsTotal = purchases.reduce((sum, currObj) => {
	return sum += currObj.owner === "Bob" ? currObj.price : 0;
}, 0);
